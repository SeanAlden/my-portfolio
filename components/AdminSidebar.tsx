...
import Link from 'next/link';
import { 
  LogOut, LayoutDashboard, Briefcase, GraduationCap, Code, 
  MessageSquare, Settings, ListTree, Zap, Building, Loader2, User
} from 'lucide-react';
...
export default function AdminSidebar() {
  ...
  const [isLoggingOut, setIsLoggingOut] = useState(false);
  const [isLogoutModalOpen, setIsLogoutModalOpen] = useState(false); 
  const [userProfile, setUserProfile] = useState<any>(null); // State untuk data user

  // Mengambil data user saat sidebar dimuat
  useEffect(() => {
    const token = localStorage.getItem('admin_token');
  ...
  return (
    <>
      <aside className="w-64 bg-white border-r border-gray-200 flex-col hidden md:flex h-screen sticky top-0">
        ...
        {/* HEADER PROFIL KLIKABEL */}
        <div className="p-4 border-b border-gray-100">
          <Link 
            href="/admin/profile" 
            className="flex items-center space-x-3 w-full p-2 hover:bg-blue-50 rounded-xl transition group"
            title="Pengaturan Profil"
          >
            <div className="relative w-11 h-11 rounded-full overflow-hidden shrink-0 shadow-sm border-2 border-gray-100 group-hover:border-blue-200 bg-gray-50 flex items-center justify-center">
              {userProfile?.image_url ? (
                <img src={userProfile.image_url} alt="Profile" className="w-full h-full object-cover" />
              ) : (
                <User className="w-5 h-5 text-gray-400" />
              )}
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="text-sm font-bold text-gray-900 truncate group-hover:text-blue-700 transition">
